...
//    其中，网关地址长度为1字节，网关地址内容为变长，实际长度由网关地址长度决定。
    ...
    private void initBySSALConfigProperties(String targetIp, int targetPort, String administrativeOrLogicalAddressHex, UserContentTypeOctEnum userContentType) {
        ...
        this.setFunctionCode(FunctionCode.giveMeOne(userContentType));
        this.setSsalVersion(SSALVersion.giveMeOne());
        this.setDeviceAddressType(DeviceAddressType.giveMeOne());
        this.setDeviceAddress(DeviceAddress.giveMeOne(administrativeOrLogicalAddressHex));
...
//        no communicate Info
    ...
    //    上行
    public static ControlData giveMeOne(String targetIp, int targetPort, UserContentTypeOctEnum userContentType) {
        return giveMeOne(targetIp, targetPort, SSALConfigInitProperties.pspMACAddress, userContentType);
    }

    //    下行/上行
    public static ControlData giveMeOne(String targetIp, int targetPort, String administrativeOrLogicalAddressHex, UserContentTypeOctEnum userContentType) {
        ...
        if (!instanceStore.containsKey(key)) {
            ...
            instance.initBySSALConfigProperties(targetIp, targetPort, administrativeOrLogicalAddressHex, userContentType);
    ...
    public String getControlDataHexForHcsCheck() {
        String controlDataHex = StringUtils.EMPTY;
        if (this.getFunctionCode() != null) controlDataHex += this.getFunctionCode().getFunctionCodeHex();
        if (this.getSsalVersion() != null) controlDataHex += this.getSsalVersion().getSsalVersionHex();
        if (this.getDeviceAddressType() != null) controlDataHex += this.getDeviceAddressType().getDeviceAddressTypeHexRevrs();
        if (this.getDeviceAddress() != null) controlDataHex += this.getDeviceAddress().getDeviceAddressHex();
        if (this.getSourceAddress() != null) controlDataHex += this.getSourceAddress().getSourceAddressHex();
        if (this.getTargetAddress() != null) controlDataHex += this.getTargetAddress().getTargetAddressHex();
        if (this.getCommunicateInfo() != null) controlDataHex += this.getCommunicateInfo().getCommunicateInfoHex();
        if (this.getSsalTimestamp() != null) controlDataHex += this.getSsalTimestamp().getTimestampHex();
        if (this.getGateAddress() != null) controlDataHex += this.getGateAddress().getGateAddressOriginalHex();
        return controlDataHex;
    }
}
