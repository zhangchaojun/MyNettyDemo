...
public class FunctionCode implements Cloneable {
    ...
    private static Map<String, FunctionCode> instanceStore = new HashMap<>();
...
//    针对230终端，24h密钥过期后，网关不主动触发会话密钥协商, 建议网关是否自动可配，主站选择触发会话协商。
    ...
    //    用系统参数创建一个对象并返回
    private void initBySSALConfigProperties(UserContentTypeOctEnum userContentTypeOct) {
        ...
        this.setUserContentTypeOct(userContentTypeOct);
    ...
    public static FunctionCode giveMeOne(UserContentTypeOctEnum userContentTypeOct) {
        if (!instanceStore.containsKey(userContentTypeOct.getValue())) {
            FunctionCode instance = new FunctionCode();
            instance.initBySSALConfigProperties(userContentTypeOct);
            instanceStore.put(userContentTypeOct.getValue(), instance);
            return instance.copy();
        }else {
            return instanceStore.get(userContentTypeOct.getValue()).copy();
        }
