...
public class SSALVersion implements Cloneable {
    ...
    private void initBySSALConfigProperties() {
        ...
        String lower8bit = this.getRetain() + StringUtils.leftPad(Integer.toBinaryString(Integer.parseInt(SSALConfigInitProperties.encryptAlgorithm.getValue())), 4, '0');
        String lower8bitHex = ByteTools.int2HexStr(Integer.parseInt(lower8bit, 2));
        String hight8bitHex = this.getVersionHex();
        this.setSsalVersionHex(lower8bitHex + hight8bitHex);
    }

    public static synchronized SSALVersion giveMeOne() {
        if (instance == null) {
