...
//     * 循环冗余检验：CRC-16-CCITT查表法
    ...
    public static byte[] CRC16_ccitt(byte[] pSrcData) {
        int crc_reg = 0xffff;
        for (int i = 0; i < pSrcData.length; i++) {
            crc_reg = (crc_reg >> 8) ^ CRC16_ccitt_table[(crc_reg ^ pSrcData[i]) & 0xFF];
        }
        crc_reg = (crc_reg ^ 0xffff);
        byte[] data1 = new byte[2];
        data1[0] = (byte) (crc_reg & 0xff);
        data1[1] = (byte) ((crc_reg >> 8) & 0xff);
        return data1;
    }
